...
class DomainHandleTask(BaseHandleTask):

    name = 'master_domain_handler'

    def get_target_list(self, target_option):
        db.session.close()
        target_list = [{'uid': domain.uid, 'target': domain.name}
                       for domain in Domain.list_items_paginate_by_search(**target_option).items]
        return target_list
